...
trait ValueBinderInstances extends ParameterTypeMetaInstances {
  ...
  implicit def traversableBinder[A : ValueBinder : ParameterTypeMeta, T <: Traversable[A]]: ValueBinder[T] = {
    val binder = implicitly[ValueBinder[A]]
    new ValueBinder[T] {
  ...
  implicit def seqBinder[A : ValueBinder : ParameterTypeMeta]: ValueBinder[Seq[A]] = traversableBinder[A, Seq[A]]
  implicit def listBinder[A : ValueBinder : ParameterTypeMeta]: ValueBinder[List[A]] = traversableBinder[A, List[A]]
  implicit def vectorBinder[A : ValueBinder : ParameterTypeMeta]: ValueBinder[Vector[A]] = traversableBinder[A, Vector[A]]
